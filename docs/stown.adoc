...
To install the https://pypi.org/project/stown/[PyPI package], use the pip
utility.
...
of name collisions, later changes may not override earlier ones unless given
permission via the `--force` or `--override` options. The latter requires a
regular expression (RE) understood by Python (see
https://docs.python.org/3/library/re.html#regular-expression-syntax[syntax]):

```bash
# Use quotes to protect RE from shell interpolation
stown --override '\.(json|toml)$' foo bar
```

...
The prefix `dot-` in source names is converted to a leading dot character in
symlinks, as shown above with `zulu/.editorconfig` pointing to
`bravo/dot-editorconfig`. This makes handling dotfiles easier, disabling their
respective special effects and lifting their "hidden" status.
